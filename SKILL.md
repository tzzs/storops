...
which scripts exist. Prefer following the workflows below over calling
scripts ad hoc; the user should not need to know script names.
...
2. Everything defaults to read-only. `scan.ps1`, `inspect.ps1`, `search.ps1`,
   `identify.ps1`, `cleanup-plan.ps1`, and `migrate-plan.ps1` never modify
   the filesystem -- run them freely, without asking first.
3. Never guess what a path is from its name. Only `identify.ps1` (backed by
   `rules/*.yaml`) determines category/risk/deletability. A path with no
...
4. Never treat "cache" or "temp" in a name as license to delete. Only act on
   what `identify.ps1`/`cleanup-plan.ps1` actually classified.
5. Critical-risk paths (Windows, Program Files, unknown system files, user
   ...
   in code (`Assert-StorOpsNotCritical`), not just by convention -- don't try
   to route around it.
6. Every write operation (`cleanup-execute.ps1`, `migrate-execute.ps1`)
   requires the user's explicit, itemized confirmation first, and the
   scripts themselves refuse to run without `-Confirm`. Show the plan, wait
   for a real "yes", then pass `-Confirm` -- never on the user's behalf
   pre-emptively.
7. If an application may be running, do not move its data. `migrate-plan.ps1`
   flags `RequiresAppClosed`; `migrate-execute.ps1` refuses to run without an
   explicit `-AppClosed` acknowledgement in that case. Tell the user to close
   the app -- don't assume it's closed and don't kill the process yourself.
...
10. Prefer an application's own config-based relocation over a Junction.
    `migrate-plan.ps1` already encodes this precedence -- don't override it
    towards Junction just because it seems simpler.
11. Verify after every migration (`verify.ps1` against the result file
...
1. `scan.ps1 -Path C:\` (or the drive the user mentioned) for top-level
   consumers and free space.
2. For any large and unidentified or ambiguous entry, `inspect.ps1` into it
   to see what's actually inside.
3. Cross-reference every notable entry with `identify.ps1` (scan/inspect
...
2. Run `cleanup-plan.ps1` (default `-MaxRisk low`; only raise it if the user
   explicitly says they're fine with medium/high-risk items too). This
   produces an itemized JSON plan -- nothing is deleted yet.
...
4. Ask the user to confirm. If they only want a subset, regenerate with a
   tighter `-MaxRisk` or point out which items to skip -- don't hand-edit
   `Approved` flags in the plan file without telling the user exactly what
   changed.
5. Only once the user confirms, run `cleanup-execute.ps1 -PlanFile <path>
   -Confirm`.
...
1. Identify the source path (`identify.ps1` if not already known from a
   scan). If it's not `Migratable`, say so and explain why (e.g. critical,
   or no known migration path) instead of improvising one.
2. Run `migrate-plan.ps1 -Path <source> -Destination <target>`. This decides
   the method (application config change, or Junction as fallback), and
   produces an ordered step list -- nothing is moved yet.
3. Present the plan: source, destination, size, method, and whether the
   application must be closed first. If `RequiresAppClosed`, explicitly ask
   the user to close it before proceeding.
4. Ask for confirmation on the plan as a whole.
5. Run `migrate-execute.ps1 -PlanFile <path> -Confirm` (add `-AppClosed` once
   the user has confirmed the app is closed). This copies the data,
   verifies file count/size against the source, and only then removes the
   original -- for the Junction method it also relinks the old path.
...
7. Run `verify.ps1 -ResultFile <path from migrate-execute output>` and report
   PASS/FAIL per check. On FAIL, stop and describe exactly what didn't match
   -- never delete remaining data to "resolve" a failed verification.

## Workflow: "is it safe to delete/move <path>?"

Just run `identify.ps1 -Path <path>` and relay Category, Deletable,
